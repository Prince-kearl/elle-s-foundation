          ...
          {eventsLoading ? <EmptyState label="Loading events…" /> : orderedEvents.length === 0 ? <EmptyState label="No events yet. Add the first community event." /> : (
            <div className="divide-y divide-[#e3e8e4]">
              {orderedEvents.map((event) => (
                <div key={event.id} className="grid gap-4 p-5 md:grid-cols-[4.5rem_1fr_auto] md:items-center hover:bg-[var(--background)]">
                  <div className="grid size-14 place-items-center border border-[#0b4a5a]/15 bg-[var(--cream)] text-center"><div><div className="font-display text-2xl font-semibold leading-none text-[#0b4a5a]">{new Date(`${event.event_date}T00:00:00`).toLocaleDateString("en-GH", { day: "2-digit" })}</div><div className="mt-1 text-[0.58rem] font-bold uppercase tracking-[0.16em] text-[#f26518]">{new Date(`${event.event_date}T00:00:00`).toLocaleDateString("en-GH", { month: "short" })}</div></div></div>
                  <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="text-[0.6rem] font-bold uppercase tracking-[0.14em] text-[var(--muted-foreground)]">{event.event_type}</span>{event.status === "published" && event.visible ? <Badge tone="success">Published</Badge> : event.status === "archived" ? <Badge>Archived</Badge> : <Badge tone="warn">Draft</Badge>}</div><h3 className="mt-1 font-display text-xl font-semibold text-[#0b4a5a]">{event.title}</h3><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-[var(--muted-foreground)]"><span className="inline-flex items-center gap-1.5"><MapPin className="size-3.5" /> {event.location}</span><span>{event.start_time ? event.start_time.slice(0, 5) : "Time to be confirmed"}</span></div></div>
...
function Metric({ icon: Icon, label, value, tone }: { icon: typeof CalendarDays; label: string; value: number; tone: "gold" | "green" | "orange" | "blue" }) {
  const tones = { gold: "bg-[#fff7c7] text-[#0b4a5a]", green: "bg-[#dff5e8] text-[var(--primary)]", orange: "bg-[#ffe2d2] text-[#f26518]", blue: "bg-[#dcecf4] text-[#1b86b8]" };
  return <div className="border border-[#dfe6e1] bg-white p-4"><div className="flex items-start justify-between"><span className={`grid size-9 place-items-center ${tones[tone]}`}><Icon className="size-4" /></span><span className="text-[0.58rem] font-bold uppercase tracking-[0.14em] text-[var(--muted-foreground)]">Live</span></div><div className="mt-4 font-display text-3xl font-semibold text-[#0b4a5a]">{value}</div><div className="mt-1 text-[0.62rem] font-bold uppercase tracking-[0.12em] text-[var(--muted-foreground)]">{label}</div></div>;
}

function SectionHeader({ eyebrow, title, icon }: { eyebrow: string; title: string; icon: React.ReactNode }) {
  return <div className="flex items-center justify-between gap-3 border-b border-[#e3e8e4] p-5"><div><div className="text-[0.58rem] font-bold uppercase tracking-[0.16em] text-[var(--muted-foreground)]">{eyebrow}</div><h2 className="mt-1 font-display text-xl font-semibold text-[#0b4a5a]">{title}</h2></div>{icon}</div>;
}

function EmptyState({ label }: { label: string }) {
  return <div className="border-b border-dashed border-[#dfe6e1] p-10 text-center text-sm text-[var(--muted-foreground)]">{label}</div>;
}
...
function EventEditor({ event, onClose, onSave }: { event: Partial<EventRecord>; onClose: () => void; onSave: (event: Partial<EventRecord>) => Promise<void> }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[var(--forest)]/70 p-4" role="dialog" aria-modal="true" onClick={onClose}><div className="w-full max-w-2xl border border-[#0b4a5a]/20 bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}><div className="flex items-start justify-between border-b border-[#dfe6e1] p-6"><div><div className="text-[0.58rem] font-bold uppercase tracking-[0.16em] text-[#f26518]">Calendar manager</div><h2 className="mt-2 font-display text-2xl font-semibold text-[#0b4a5a]">{state.id ? "Edit event" : "Create event"}</h2></div><button type="button" onClick={onClose} className="border border-[#0b4a5a]/15 px-3 py-2 text-sm font-bold text-[#0b4a5a]">Esc</button></div><form onSubmit={(e) => { e.preventDefault(); onSave(state); }} className="grid gap-4 p-6 sm:grid-cols-2"><label className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted-foreground)] sm:col-span-2">Event title<TextInput required value={state.title ?? ""} onChange={(e) => set("title", e.target.value)} className="mt-2" placeholder="The Smile Project" /></label><label className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted-foreground)]">Event type<TextInput required value={state.event_type ?? ""} onChange={(e) => set("event_type", e.target.value)} className="mt-2" placeholder="Community event" /></label><label className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted-foreground)]">Location<TextInput required value={state.location ?? ""} onChange={(e) => set("location", e.target.value)} className="mt-2" placeholder="Dzowulu Special School" /></label><label className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted-foreground)]">Date<TextInput required type="date" value={state.event_date ?? ""} onChange={(e) => set("event_date", e.target.value)} className="mt-2" /></label><label className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted-foreground)]">Start time<TextInput type="time" value={state.start_time ?? ""} onChange={(e) => set("start_time", e.target.value)} className="mt-2" /></label><label className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted-foreground)]">End time<TextInput type="time" value={state.end_time ?? ""} onChange={(e) => set("end_time", e.target.value)} className="mt-2" /></label><label className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted-foreground)]">Status<select value={state.status ?? "draft"} onChange={(e) => set("status", e.target.value)} className="mt-2 w-full border border-[#0b4a5a]/15 bg-white px-3 py-2.5 text-sm font-normal normal-case tracking-normal outline-none focus:border-[#f26518]"><option value="draft">Draft</option><option value="published">Published</option><option value="archived">Archived</option></select></label><label className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted-foreground)] sm:col-span-2">Description<div className="mt-2"><RichTextEditor value={state.description ?? ""} onChange={(value) => set("description", value)} placeholder="Describe the event and what guests can expect." /></div></label><div className="flex items-center justify-between gap-3 border-t border-[#dfe6e1] pt-4 sm:col-span-2"><label className="flex items-center gap-2 text-sm font-semibold text-[#0b4a5a]"><input type="checkbox" checked={!!state.visible} onChange={(e) => set("visible", e.target.checked)} /> Show on public calendar</label><div className="flex gap-2"><GhostButton onClick={onClose}>Cancel</GhostButton><PrimaryButton type="submit">Save event</PrimaryButton></div></div></form></div></div>;
}
